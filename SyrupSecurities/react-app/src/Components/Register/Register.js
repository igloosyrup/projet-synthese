...
const Register = () => {
    ...
    const registerNewCart = async (customer) => {
        const result = await fetch('http://localhost:8888/cart/new',
            {
                method: 'POST',
                headers: {
                    'Content-type': 'application/json'
    ...
    const onSubmit = (e) => {
        ...
        registerCustomer(customer)
            .then((res) => res.id != null ? registerNewCart(res)
                .then((cartRes) => cartRes.id != null ? registerSuccess() : window.alert("Erreur D'enregistrement"))
